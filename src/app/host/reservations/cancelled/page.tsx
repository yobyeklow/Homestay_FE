...
const AllCancelledReservation = () => {
  const customerID = localStorage.getItem("customerID");
  const pathname = usePathname();
  const { data, isLoading } = useQuery(["CancelledReservation"], async () => {
    const response = await axios.get(
      `/api/booking/get-all/canceled/${customerID}`
    );
      ...
      <main className="reservationPage w-full h-full px-8 mt-16">
        ...
        <div className="wrapper w-full border-b-gray-300 border-b-[1px]">
          <div className="header-table flex items-center overflow-visible">
            ...
            <span
              className={` ${
                pathname === "/host/reservations/cancelled"
...
export default AllCancelledReservation;
